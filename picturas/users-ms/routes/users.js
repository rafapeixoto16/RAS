...
router.get('/:id', (req, res) => {
    User.getUser(req.params.id)
        .then((resp) => res.status.jsonp(resp))
        .catch((err) => res.status(446).render('error', { message: err }));
});

router.put('/:id/update', (req, res) => {
    User.updateUser(req.params.id, req.body)
        .then((resp) => res.status.jsonp(resp))
        .catch((err) => res.status(447).render('error', { message: err }));
});
