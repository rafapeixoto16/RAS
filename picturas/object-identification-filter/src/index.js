...
async function detectObjects(imageBuffer, minScore = 0.3) {
  ...
  const resizedImage = tf.image.resizeBilinear(image, [targetHeight, targetWidth]);
...
async function objectIdentificationHandler(imageBuffer, params) {
  ...
  return await drawBoundingBoxes(imageBuffer, predictions);
}

createFilterHandler('object-identification', objIdentificationSchema, objectIdentificationHandler);
