...
const createAndAddWorker = async (language) => {
  try {
    ...
    throw new Error(`Language '${language}' is not supported or failed to load.`);
  }
};
...
setInterval(() => {
  resetScheduler()
    .then(() => console.log("Scheduler reset completed."))
    .catch((err) => console.error("Error during scheduler reset:", err));
