<template>
  <div class="min-h-screen bg-gradient-to-br from-blue-50 to-white relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
      <h1 class="text-5xl font-bold mb-8 text-center text-transparent bg-clip-text bg-gradient-to-r from-[#3B82F6] to-purple-600 font-caveat px-4 py-2">Picturas</h1>
        ...
        <div class="flex flex-col lg:flex-row lg:space-x-16">
          <div class="w-full lg:w-1/3 flex justify-center items-start mb-8 lg:mb-0">
            <div class="relative group">
              ...
              <img v-else :src="user.avatarUrl" :alt="user.username"
          ...
          <div class="w-full lg:w-2/3">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
              <div v-for="(item, index) in userInfoItems" :key="index" class="flex flex-col space-y-2"
                ...
                <input v-if="isEditing && item.editable && item.key !== 'bio'" :value="item.value"
                ...
                <textarea v-else-if="isEditing && item.editable && item.key === 'bio'" :value="item.value"
                  :placeholder="`Enter your ${item.label.toLowerCase()}`"
                  @input="updateUserInfo(item.key, ($event.target as HTMLTextAreaElement).value)" rows="4"
                  class="w-full bg-gray-50 border-2 border-gray-200 rounded-lg px-6 py-4 text-lg lg:text-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300"></textarea>
                <div v-else class="text-gray-800 text-lg lg:text-xl py-4 bg-gray-50 rounded-lg px-6"
    ...
    <ConfirmationModal v-if="showConfirmationModal" :changes="changedFields" @confirm="confirmChanges"
